...
import { TAdmin, TBloodGroup, TGender, TUserName } from './admin.interfaces';
...
adminModelSchema.pre('find', function (next) {
  this.find({ isDeleted: { $ne: true } });
  next();
});

adminModelSchema.pre('findOne', function (next) {
  this.find({ isDeleted: { $ne: true } });
  next();
});

adminModelSchema.pre('aggregate', function (next) {
  this.pipeline().unshift({ $match: { isDeleted: { $ne: true } } });
  next();
});

//checking if user is already exist!
adminModelSchema.statics.isUserExists = async function (id: string) {
